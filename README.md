...
## معماری
- **خواندن پیام (`reader.py`)** — روی GitHub Actions اجرا می‌شود. **دیگه با `schedule` خودِ
  گیت‌هاب صدا زده نمی‌شه** — چون در عمل معلوم شد گیت‌هاب زمان‌بندی‌هاش رو با تاخیرهای
  غیرقابل‌پیش‌بینی (گاهی ۱-۲ ساعت!) اجرا می‌کنه. به‌جاش، Cloudflare Worker با یه Cron Trigger
  دقیق‌تر، هر ۳۰ دقیقه، یه فراخوانی API به گیت‌هاب می‌زنه (`workflow_dispatch`) و ورک‌فلو رو
  مستقیم اجرا می‌کنه. با اکانت شخصی (Telethon) آخرین پیام‌های ربات منبع را می‌خواند و بر
  اساس محتوای تاریخ/ساعت (نه شناسه‌ی پیام) تشخیص می‌دهد که آیا این خاموشی قبلاً فرستاده شده
  یا نه، تا از تکرار جلوگیری شود.
- **ربات مدیریت (`worker/`)** — روی Cloudflare Workers، به‌صورت Webhook. حالا خودش یک عضو
  واقعی گروه‌هاست: پیش‌نمایش با دکمه برای شما می‌فرستد، دستورات را مدیریت می‌کند، با یک Cron
  Trigger هر ۲ دقیقه تایم‌اوت‌ها و پاکسازی گزارش‌ها را چک می‌کند، با یک Cron Trigger دیگر هر
  ۳۰ دقیقه ورک‌فلوی گیت‌هاب را صدا می‌زند، و وقتی تایید کنید یا تایم‌اوت بخورد، **خودش
...
- `GITHUB_TOKEN`/`GITHUB_REPO` روی Worker حالا برای دو کار لازمند: صدا زدن ورک‌فلوی گیت‌هاب
  هر ۳۰ دقیقه، و قابلیت `/setsecret` (پایین‌تر). مطمئن شو `GITHUB_TOKEN` دسترسی
  **Actions: Read and write** رو داره.
...
npx wrangler secret put GITHUB_TOKEN          # حالا هم برای صدا زدن ورک‌فلو هر ۳۰ دقیقه لازمه، هم /setsecret
npx wrangler secret put GITHUB_REPO           # مثلا yourusername/bale-bargh-bot
# اختیاری — فقط اگه اسم برنچ پیش‌فرض ریپوت "main" نیست:
npx wrangler secret put GITHUB_BRANCH

npx wrangler deploy
```

⚠️ برای این‌که `GITHUB_TOKEN` بتونه ورک‌فلو رو صدا بزنه، باید دسترسی **Actions: Read and
write** رو داشته باشه (همونی که برای `/setsecret` هم لازم بود، پس اگه قبلاً درستش کردی
...
## گزارش مانیتورینگ (بخش «گزارش اجراها»)
`reader.py` بعد از هر اجرا (هر ۳۰ دقیقه، حتی وقتی هیچ اتفاق خاصی نیفتاده) یه خلاصه‌ی کامل
از کاری که انجام داد رو به Worker می‌فرسته، ولی به‌جای اینکه هر بار توی چتت پیام جدید
بفرسته، این گزارش‌ها توی Cloudflare KV ذخیره می‌شن. هروقت خواستی، از دکمه‌ی «🛠 گزارش
اجراها» توی منوی اصلی (یا با فرستادن `/logs`) آخرین ۱۰ گزارش رو می‌بینی. خودِ این بخش هم
هر ۳ روز یک‌بار خودش پاک می‌شه (توسط همون Cron Trigger که تایم‌اوت‌ها رو چک می‌کنه) تا
شلوغ نشه.
